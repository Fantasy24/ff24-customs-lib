      ...
      <el-card>
        <div class="title-container">
          <img
              ...
              width="100"
          >
          <h3 class="title">{{ $t('login.activeAccount').toUpperCase() }}</h3>
          <el-tooltip
              :content="$t('login.changeLang')"
              placement="top"
          >
            <lang-select class="set-language" :i18n="i18n" :store="store"/>
          </el-tooltip>
        </div>
        ...
        <el-tooltip v-model="capsTooltip" :content="$t('login.checkCaplock')" manual placement="right">
          <el-form-item prop="newPassword">
            ...
            <el-input
                :key="passwordType"
                ref="newPassword"
                v-model="formActive.newPassword"
                :maxlength="50"
        ...
        <el-form-item prop="confirmPassword">
          ...
          <el-input
              ...
              ref="confirmPassword"
              v-model="formActive.confirmPassword"
              :maxlength="50"
...
export default {
  name: 'AccountActive',
  components: {RuleChangePass, LangSelect},
  data() {
    return {
      formActive: {
  ...
  mounted() {
    if (this.$refs.newPassword && this.formActive.newPassword === '') {
      this.$refs.newPassword.focus();
    } else if (this.$refs.confirmPassword && this.formActive.confirmPassword === '') {
  ...
  methods: {
    onChangePass() {
      this.$refs.changePassForm.validate((valid) => {
        ...
        apiFactory.callAPI(ConstantAPI.ACCOUNT_CHANGE.ACTIVE_PASSWORD, this.formActive).then(() => {
          removeToken();
          this.router.push('/login?redirect=/dashboard');
          showAlert(this, SUCCESS, `Kích hoạt thành công : Tài khoản [${this.formActive.username}]`);
        }).catch(err => {
          errAlert(this, err);
        });
      });
