...
export default {
  ...
  data() {
    ...
    const validateNewPass = (rule, value, callback) => {
      ...
      const uid = this.$store.getters.uid.toLowerCase();
      if (value === "") {
        callback(new Error("Nhập mật khẩu mới"));
      } else if (value.length < passLenght) {
        callback(new Error(`Tối thiểu ${passLenght} ký tự`));
      } else if (value.toLowerCase().includes(uid)) {
        callback(
          new Error(
  ...
  methods: {
    onChangePass() {
      this.$refs["formChangePass"].validate((valid) => {
        ...
        this.loading = true;
        this.formChangePass.username = this.$store.getters.uid; //this.user.uid;
        apiFactory
          .callAPI(
            ConstantAPI.ACCOUNT_CHANGE.CHANGE_PASSWORD,
            this.formChangePass
          )
          .then(async (_) => {
          ...
          .catch((err) => {
            if (
              ...
              err.response.data.name.includes("must be")
            ) {
              return this.$message({
            ...
            if (
              err.response.data.status === 400 &&
              err.response.data.code.includes("EQUALFIELD")
            ) {
              return this.$message({
    ...
    onSendCodeVerifyToEmail() {
      apiFactory
        .callAPI(ConstantAPI["ql-nguoi-dung"].GET_RESEND_TOTP_KEY)
        .then(() => {
          this.counting = true;
          showAlert(
            this,
            SUCCESS,
            "Đã gửi mã xác thực, vui lòng kiểm tra email của bạn"
          );
        })
        .catch((err) => {
          errAlert(this, err);
