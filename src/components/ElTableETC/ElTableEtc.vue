<template>
  <div>
    ...
    <el-card v-if="isShowUtil && (isExport || isShowPdf || isShowPrint || isShowHideColumn)" id="formUtil"
             class="utilStyle" shadow="never"
    >
      <div>
...
import Vue from 'vue'
import DropdownExport from './DropdownExport.vue'

Vue.use(ReadMore)
export default {
  name: 'ElTableEtc',
  components: {Print, Pagination, DropdownExport},
  props: {
    tableSize: {
      type: String,
      default: 'mini'
  ...
  methods: {
    search(successCallback = () => {
      ...
      apiFactory.callAPI(this.apiFetch, {}, paramPaging).then(rs => {
        this.syncData = rs.result || rs[this.rsProp].result
        this.syncTotal = (rs.totalElements === null || rs.totalElements === undefined) ? rs[this.rsProp].totalElements : rs.totalElements
        successCallback(rs)
      }).catch(err => {
        errAlert(this, err)
        failCallback(err)
    ...
    pagination() {
      ...
      this.paramsFetch.size = this.size
      this.$emit('update:paramsFetch', this.paramsFetch)
      apiFactory.callAPI(this.apiFetch, {}, paramPaging).then(rs => {
        let data = rs.result || rs[this.rsProp].result

        this.$emit('pagination-success', data)

        if (data && data.length) {
          data.forEach(obj => {
            for (const objKey in obj) {
              if (obj.hasOwnProperty(objKey)) {
                obj[objKey] = (obj[objKey] || obj[objKey] === 0) ? obj[objKey] : ''
              }
            }
          })
        }
        this.syncLoadData = false
        this.syncData = data
        this.syncTotal = rs.totalElements || rs[this.rsProp].totalElements
      }).catch(err => {
        this.syncLoadData = false
        errAlert(this, err)
      })
    ...
    handleMinWidth(col) {
      ...
      if (typeof minWidth === 'string' && !minWidth.includes('px')) {
        return `${minWidth}px`
      } else {
        return minWidth || col.width
      }
    },
